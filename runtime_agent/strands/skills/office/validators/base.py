...
Canonical shared implementation used by docx/xlsx/pptx skill validators.
"""
...
# OOXML element names ending with these suffixes map to relationship types by
# stripping the suffix (e.g. "sldId" -> "slide", "chartReference" -> "chart").
_RELATIONSHIP_ID_SUFFIX = "id"
_RELATIONSHIP_ID_SUFFIX_LEN = len(_RELATIONSHIP_ID_SUFFIX)  # 2
_RELATIONSHIP_REFERENCE_SUFFIX = "reference"
_RELATIONSHIP_REFERENCE_SUFFIX_LEN = len(_RELATIONSHIP_REFERENCE_SUFFIX)  # 9
...
class BaseSchemaValidator:
    ...
    def __init__(self, unpacked_dir, original_file=None, verbose=False):
        ...
        # Resolve schemas from the concrete skill package (docx/xlsx/pptx),
        # not from this shared module's location.
        subclass_file = Path(inspect.getfile(type(self))).resolve()
        self.schemas_dir = subclass_file.parent.parent / "schemas"

        patterns = ["*.xml", "*.rels"]
        self.xml_files = [
            xml_path
            for pattern in patterns
            for xml_path in self.unpacked_dir.rglob(pattern)
        ]
    ...
    def repair_whitespace_preservation(self) -> int:
        ...
        for xml_file in self.xml_files:
            try:
                ...
                for element in dom.getElementsByTagName("*"):
                    ...
                    if local_name in ("t", "delText", "instrText", "delInstrText"):
                        text = "".join(
                            ...
                            in (text_node.TEXT_NODE, text_node.CDATA_SECTION_NODE)
                        )
                        ws = (" ", "\t", "\n", "\r")
                        if text and (text.startswith(ws) or text.endswith(ws)):
                            if element.getAttribute("xml:space") != "preserve":
                                ...
                                pending.append(
                                    f"  Repaired: {xml_file.name}: Added xml:space='preserve' "
                                    f"to {element.tagName}: {text_preview}"
                                )
                ...
                logger.warning(
                    "Whitespace repair failed for %s", xml_file.name, exc_info=True
                )
                continue
    ...
    def validate_xml(self):
        ...
        for xml_file in self.xml_files:
            try:
                ...
                logger.warning(
                    "XML syntax error in %s at line %s: %s",
                ...
                logger.warning(
                    "Unexpected error validating XML file %s",
                    xml_file.relative_to(self.unpacked_dir),
                    exc_info=True,
                )
                errors.append(
                    f"  {xml_file.relative_to(self.unpacked_dir)}: "
                    "Unexpected error during XML validation"
                )
    ...
    def validate_namespaces(self):
        ...
        for xml_file in self.xml_files:
            try:
                ...
                logger.warning(
                    "Skipping namespace validation for %s: XML is not well-formed",
                    xml_file.relative_to(self.unpacked_dir),
                    exc_info=True,
                )
                continue
    ...
    def validate_unique_ids(self):
        ...
        for xml_file in self.xml_files:
            try:
                ...
                logger.warning(
                    "Error validating unique IDs in %s",
                    xml_file.relative_to(self.unpacked_dir),
                    exc_info=True,
                )
                errors.append(
                    f"  {xml_file.relative_to(self.unpacked_dir)}: Error during ID validation"
                )
    ...
    def _get_expected_relationship_type(self, element_name):
        ...
        if (
            elem_lower.endswith(_RELATIONSHIP_ID_SUFFIX)
            and len(elem_lower) > _RELATIONSHIP_ID_SUFFIX_LEN
        ):
            prefix = elem_lower[:-_RELATIONSHIP_ID_SUFFIX_LEN]
        ...
        if (
            elem_lower.endswith(_RELATIONSHIP_REFERENCE_SUFFIX)
            and len(elem_lower) > _RELATIONSHIP_REFERENCE_SUFFIX_LEN
        ):
            prefix = elem_lower[:-_RELATIONSHIP_REFERENCE_SUFFIX_LEN]
            return prefix.lower()
        ...
        try:
            ...
            for xml_file in self.xml_files:
                ...
                try:
                    ...
                    logger.warning(
                        "Skipping content-type check for %s", path_str, exc_info=True
                    )
                    continue
            ...
            logger.warning("Error parsing [Content_Types].xml", exc_info=True)
    ...
    def _validate_single_file_xsd(self, xml_file, base_path, schema_path=None):
        ...
        try:
            ...
            with open(xml_file, "r") as xml_file_handle:
                xml_doc = lxml.etree.parse(xml_file_handle)
