...
# OOXML paraId is a 32-bit hex value; values >= 0x80000000 are rejected by Word.
PARA_ID_MAX = 0x80000000
# OOXML durableId must stay below signed 32-bit max (0x7FFFFFFF).
DURABLE_ID_MAX = 0x7FFFFFFF
# Inclusive upper bound for repaired durableId values (DURABLE_ID_MAX - 1).
DURABLE_ID_REPAIR_MAX = 0x7FFFFFFE
            ...
            try:
                for elem in lxml.etree.parse(str(xml_file)).iter():
                    if val := elem.get(para_id_attr):
                        try:
                            if self._parse_id_value(val, base=16) >= PARA_ID_MAX:
                                errors.append(
                                    f"  {xml_file.name}:{elem.sourceline}: paraId={val} >= {PARA_ID_MAX:#x}"
                                )
                        except ValueError:
                            errors.append(
                    ...
                    if val := elem.get(durable_id_attr):
                        if xml_file.name == "numbering.xml":
                            try:
                                if self._parse_id_value(val, base=10) >= DURABLE_ID_MAX:
                                    errors.append(
                                        f"  {xml_file.name}:{elem.sourceline}: "
                                        f"durableId={val} >= {DURABLE_ID_MAX:#x}"
                                    )
                            except ValueError:
                                errors.append(
                                    f"  {xml_file.name}:{elem.sourceline}: "
                            ...
                            try:
                                if self._parse_id_value(val, base=16) >= DURABLE_ID_MAX:
                                    errors.append(
                                        f"  {xml_file.name}:{elem.sourceline}: "
                                        f"durableId={val} >= {DURABLE_ID_MAX:#x}"
                                    )
                ...
                for elem in dom.getElementsByTagName("*"):
                    for attr_name in DURABLE_ID_ATTRS:
                        ...
                        try:
                            ...
                            needs_repair = key >= DURABLE_ID_MAX
                        except ValueError:
                            key = durable_id
                            needs_repair = True
                        ...
                        if needs_repair:
                            if key in seen_in_file:
                                value = random.randint(1, DURABLE_ID_REPAIR_MAX)
                            else:
                                seen_in_file.add(key)
                                if key not in renames:
                                    renames[key] = random.randint(1, DURABLE_ID_REPAIR_MAX)
                                value = renames[key]
