"""Runtime wrapper for graph node embeddings (hybrid wiki / memory search).

Canonical implementation lives in ``graph/lib/embeddings.py`` (bundled in the
Runtime Docker image). Uses ``/app/config.json`` via ``APP_CONFIG_PATH``.
"""
...
_GRAPH_DIR = Path(__file__).resolve().parent / "graph"
