...
class QueueNotificationSink:
    """Same API as :class:`NotificationQueue` but forwards to a ``queue.Queue``.

    Used by non-Streamlit callers (e.g. Flask SSE) that stream agent output to
    the client without pre-allocating ``st.empty()`` slots.
    """
