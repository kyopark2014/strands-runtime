...
- **IAM 권한**: EC2/로컬에서 installer를 실행하는 주체는 아래 작업 권한이 필요합니다.
  - S3, IAM, VPC, ECS, ECR, CloudFront, Bedrock Agent, S3 Vectors, AgentCore Control
...
- CloudFront 생성 후 `runtime_agent/strands/installer.py`를 subprocess로 실행
- ECR에 arm64 Runtime 이미지 push, AgentCore Runtime 등록
- 결과(`agent_runtime_arn` 등)는 `runtime_agent/strands/config.json` → `application/config.json`으로 병합
...
`runtime_agent/strands/installer.py`를 실행하여 AgentCore Runtime 배포
...
[8/10] 애플리케이션 설정 및 Agent Runtime 배포
       ...
       • application/config.json 생성 (sharing_url 포함)
...
삭제 순서: CloudFront → ECS (서비스/클러스터/태스크 정의/로그/ECR) → ALB → EC2(레거시) → VPC → Knowledge Base / S3 Vectors → 기타 리소스

AgentCore Runtime 삭제는 `runtime_agent/strands/uninstaller.py`를 사용합니다.
